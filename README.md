...
- 默认选择大模型牌库，可切换到本地牌库（目前本地牌库还没预设图片）。
- 支持 OpenAI、火山、千问图片模型。
- OpenAI 默认模型显示为 `ImageGen2`，实际模型 ID 为 `gpt-image-2`。（没额度，还没测试过）
- 火山默认模型显示为 `Seedream 4.5`。经多次测试，只有Seedream 5.0 Lite生成的图片满足要求，在传入参考图链接后，生成更稳定，但生成时间较久，需要耐心等待。
- 火山生图会附带一张 `Codenames: Pictures` 参考图，帮助维持卡牌可读性和卡面气质。
- 千问可选模型为 `qwen-image-2.0-pro-2026-04-22`、`qwen-image-2.0-pro`、`qwen-image-2.0-pro-2026-03-03`。别用，千问生图大模型完全达不到要求。
